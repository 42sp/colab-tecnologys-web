...
const getStatusAndProgress = (construction: Construction) => {
  switch (construction.status) {
    case 'Em Andamento':
      return { status: 'Em andamento', badgeClass: 'bg-yellow-100 text-yellow-800', progress: construction.status || 0 };
    case 'Concluído':
      return { status: 'Concluído', badgeClass: 'bg-green-100 text-green-800', progress: 100 };
    case 'Atrasado':
    default:
      return { status: 'Atrasado', badgeClass: 'bg-red-100 text-red-800', progress: construction.status || 0 };
  }
};

const DashboardTable: React.FC = () => {
  const { theme } = useTheme();
  const { constructions } = useConstructionsContext();
