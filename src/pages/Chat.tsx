...
function processListToTable(rawText: string): ChatMessage | null {
  ...
  const initialText = introMatch
    ? introMatch[1].trim()
    : "Aqui está o resumo dos serviços em andamento:";
  ...
  for (const item of listItems) {
    ...
    if (itemContent.includes("Observações:")) {
      const obsMatch = itemContent
        .substring(itemContent.indexOf("Observações:"))
        .trim();
    ...
    for (const match of pairs) {
      ...
      if (colunas.includes(key)) {
        ...
        if (
          key === "Status" &&
          linha["Observações"] !== "N/A" &&
          value.endsWith(linha["Observações"])
        ) {
...
export default function Chat() {
  ...
  async function handleSend() {
    ...
    try {
      ...
      let raw: string = ""; // --- LÓGICA DE EXTRAÇÃO RAW (Lida com String/Array) ---
      ...
      if (Array.isArray(data?.resposta) && data.resposta.length > 0) {
        ...
        raw = String(
          data?.resposta || "A chave 'resposta' está ausente ou inválida."
        );
        console.error(
          "DEBUG: 'resposta' não é uma string nem um array esperado. Retornou:",
          raw
        );
      }
      raw = raw.trim();
      ...
      if (!raw) {
        let emptyErrorMessage: ChatMessage = {
          role: "assistant",
          content:
            "A IA não retornou um texto de resposta na chave 'resposta'. Verifique a configuração do node 'Respond to Webhook' no n8n.",
        };
        setMessages((prev) => [...prev, emptyErrorMessage]);
        return;
      }

      let aiMessage: ChatMessage; // 1. Tenta detectar o formato hasTable

      if (raw.startsWith("hasTable")) {
        const lines: string[] = raw.split("\n").map((line) => line.trim());
        const textoLine = lines.find((line) => line.startsWith("texto:")) || "";
        const colunasLine =
          lines.find((line) => line.startsWith("colunas:")) || "";
        const linhasLine =
          lines.find((line) => line.startsWith("linhas:")) || "";
