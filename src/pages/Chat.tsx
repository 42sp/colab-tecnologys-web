    ...
    async function handleSend() {
        ...
        try {
            ...
            if (Array.isArray(data?.resposta) && data.resposta.length > 0) {
                ...
                rawJsonString = String(data.resposta[0].resposta || JSON.stringify(data.resposta[0]));
            ...
            if (!rawJsonString) {
                throw new Error("A IA não retornou uma string JSON válida na resposta.");
            }
            ...
            try {
                ...
                if (match && match[1]) {
                    ...
                    throw new Error("Resposta da IA não é um JSON válido e não pôde ser extraída.");
                }
            }
            ...
            if (!Array.isArray(messagesArray)) {
                 ...
                 const textMessage: ChatMessage = { 
                    role: "assistant", 
                    content: "Erro de parsing: A estrutura 'resposta_formatada.messages' não foi encontrada. Resposta bruta: " + rawJsonString 
                 };
            ...
            messagesArray.forEach((msg: any) => {
                ...
                } else if (msg.type === "table" && Array.isArray(msg.columns) && Array.isArray(msg.rows)) {
            ...
            if (newMessages.length === 0) {
                const textMessage: ChatMessage = { 
                    role: "assistant", 
                    content: "Nenhum bloco de 'text' ou 'table' válido foi encontrado na array 'messages'." 
                 };
                 setMessages(prev => [...prev, textMessage]);
                 return;
            }
            ...
            const errorMessage: ChatMessage = {
                role: "assistant",
                content: `Ocorreu um erro ao processar a resposta da IA: ${error instanceof Error ? error.message : "Erro desconhecido"}. Verifique o console para mais detalhes.`,
            };
